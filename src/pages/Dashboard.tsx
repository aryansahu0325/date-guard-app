...
export default function Dashboard() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const expiringSoon = products?.filter(p => p.expiry_date && new Date(p.expiry_date) <= thirtyDaysFromNow && new Date(p.expiry_date) >= now).length || 0;
      const warrantyExpiring = products?.filter(p => p.warranty_date && new Date(p.warranty_date) <= thirtyDaysFromNow && new Date(p.warranty_date) >= now).length || 0;
      const categories = products?.reduce((acc: {
        [key: string]: number;
      }, product) => {
        ...
        <Card>
          ...
          <CardContent>
            {products.length === 0 ? <div className="text-center py-8">
              ...
              return <div key={product.id} className="flex items-center justify-between p-4 border rounded-lg">
                      <div className="flex items-center gap-4">
                      ...
                      <div className="text-right space-y-1">
                        {expiryStatus && <div className="text-sm">
                            <Badge variant={expiryStatus.color as any} className="text-xs">
                              {expiryStatus.status === 'expired' ? `Expired ${expiryStatus.days}d ago` : `Expires in ${expiryStatus.days}d`}
                            </Badge>
                          </div>}
                        {warrantyStatus && <div className="text-sm">
                            <Badge variant={warrantyStatus.color as any} className="text-xs">
                              {warrantyStatus.status === 'expired' ? `Warranty expired ${warrantyStatus.days}d ago` : `Warranty expires in ${warrantyStatus.days}d`}
                            </Badge>
                          </div>}
                      </div>
                    </div>;
